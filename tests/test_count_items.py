from demo import count_items


def test_data_list(data_list, data_item, operation, data_results):
    res = count_items(data_list, data_item, operation=operation)
...
def test_data_invalid_parameters(data_list):
    ...
    assert count_items(data_list, "0", operation="abc") is None
    assert count_items(data_list, "27", operation="abc") is None
