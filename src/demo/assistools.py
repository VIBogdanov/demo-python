...
def rinda_multiplication(a: int, b: int) -> int:
    ...
    После второго шага получаем набор пар значенийЖ:
    (18, 89)
    (9, 178)
    (4, 356)
    (2, 712)
    ...
    # Внутренняя функция-генератор, которая выполняет попарное умножение/деление
    # на 2, и фильтрует четные значения, полученные после умножение на 2.
    def _get_addendum(a, b):
        while a > 0:
            if a & 1:
                yield b
            a >>= 1
