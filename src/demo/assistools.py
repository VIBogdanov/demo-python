...
def unpack2flat(
    *args: Any,
    not_unpack: object | Iterable[object] = ...,
    ignore: tuple = (str, bytes),
) -> Iterator[Any]:
    ...
    Args:
        args: Данные для распаковки.
        not_unpack: Типы данных, которые не нужно распаковывать.
        ignore: Игнорируемые по-умолчанию типы данных.
    ...
    match not_unpack:
        case str():
            for _obj_name in re.split(r"\W+", not_unpack):
                # Пытаемся найти объект по имени в модуле или во встроенном пространстве имен
                if _obj := globals().get(
    ...
    while iters_buff:
        ...
        for obj in iters_buff[-1]:
            ...
            if not isinstance(obj, DO_NOT_ITERATE) and (
                ...
                # Добавляем в очередь новый итератор для вложенного объекта.
