...
def get_max_from_min_difference(
    ...
    if len_data == 0 or (groups * members) > len_data or (groups * members) <= 0:
        return None
    ...
    min_key: int = members - 1  # Минимально возможное значение ключа
    max_key: int = 2 * len_data - members - 1  # Максимально возможное
    # Формируем словарь всех возможных групп заданного размера (members), одновременно сортируя
    # ключи по возрастанию величины разницы между числами в группе. В словарь отбираются только
    # те ключи, которые позволяют сформировать заданное число групп.
    # Пример 1: {2: 0, 4: 1, 16: 1, 14: 8}
    # Пример 2: {2: 0, 14: 8}
    # Пример 3: {2: 1, 4: 1, 8: 1, 10: 1}
    # Пример 4: {10: 20, 4: 30, 6: 35, 8: 35, 12: 35, 2: 40}
    diff_groups: dict[int, int] = dict(
        sorted(
            (
                (a + b, _data[b] - _data[a])
                for a, b in enumerate(range(members - 1, len_data))
                if (a + b - offset) >= min_key or (a + b + offset) <= max_key
            ),
    ...
    # Пример 3: 2
    # Пример 4: 10
    for start_key in diff_groups:
        # Ищем пару для стартового ключа с заданным смещением, которое зависит от количества и размера групп.
        # Найденная пара является той группой, разница чисел в которой и есть искомый результат.
        # Если найти такую группу невозможно, возвращаем None.
        # Пример 1: 16
        # Пример 2: 14
        # Пример 3: 8
        # Пример 4: 4
        return diff_groups.get(
            next((key for key in diff_groups if abs(key - start_key) >= offset), -1),
            None,
        )
