...
def get_max_from_min_difference(
    ...
    Details:
        Формируется словарь всех возможных групп. В каждой группе числа отсортированы. Значения словаря - это
        разница между последним и первым числом в группе. Ключи словаря - сумма первого и последнего индекса
        группы в исходном предварительно отсортированном массиве данных. Ключи дополнительно играют роль
        определителей пересечения диапазонов индексов групп. Одно и то же число может содержаться в нескольких
        группах. Необходимо будет отбирать группы с непересекающимися диапазонами чисел, для чего понадобятся
        ключи словаря и шаг непересекающихся диапазонов.
    ...
    # группе были отсортированы
    _data: list[int] = sorted(data)
    # Формируем словарь всех возможных групп заданного размера (members)
    # Пример 1: {2: 0, 4: 1, 6: 1, 8: 0, 10: 0, 12: 8, 14: 8, 16: 1}
    # Пример 2: {2: 0, 4: 1, 6: 1, 8: 0, 10: 0, 12: 8, 14: 8}
    # Пример 3: {2: 1, 4: 1, 6: 0, 8: 1, 10: 1}
    diff_ranges: dict[int, int] = {
        a + b: _data[b] - _data[a] for a, b in enumerate(range(members - 1, len(_data)))
    }
    # Отсортированный список больше не нужен
    del _data
    # Вычисляем шаг непересекающихся диапазонов. Если разница между ключами словаря больше
    # или равна вычисленному шагу, то диапазоны групп не пересекаются
    out_scope: int = 2 * members
    # Сортируем ключи словаря по его значениям, т.е. по величине разницы между числами в группах.
    # Сортировка позволит оптимизировать поиск групп с минимальной разницей Поиск слева на право.
    # Можно было использовать генератор, то тогда пришлось бы формировать олин и тот же генерато
    # два и более раз. Жертвуем памятью ради скорости,
    # Пример 1: [2, 8, 10, 4, 6, 16, 12, 14]
    # Пример 2: [2, 8, 10, 4, 6, 12, 14]
    # Пример 3: [6, 2, 4, 8, 10]
    sorted_keys: list[int] = sorted(
        diff_ranges.keys(), key=lambda _key: diff_ranges[_key]
    )
    result: int | None = None
    # Последовательно выбираем группу с наименьшей разницей и формируем для нее перечень
    # непересекающихся групп с наименьшими возможными разницами
    # Пример 1: 2
    # Пример 2: 2
    # Пример 3: Первая итерация - 6, вторая - 2
    for start_key in sorted_keys:
        result_key = start_key
        try:
            # Количество шагов по генератору ограничено заданным числом групп (groups)
            # Первая группа уже известна - это start_key, потому range стартует со
            # следующего индекса - с единицы, т.е. генератор возвращает меньшее на
            # единицу количество групп. "Лишние" группы игнорируются. Если генератор
            # не может сформировать заданное число групп, возникает исключение StopIteration.
            for _ in range(1, groups):
                result_key: int = next(
                    # Внешний генератор вычисляет следующую непересекающуюся группу с минимальной
                    # разницей относительно сохраненной в result_key предыдущей непересекающейся
                    # группы с минимальной разницей
                    # Пример 1: [8, 16]
                    # Пример 2: [8, 14]
                    # Пример 3: Первая итерация - [], вторая - [8]
                    next_key
                    for next_key in (
                        # Внутренний генератор формирует перечень всех непересекающихся групп
                        # для отобранной в начале цикла группы (start_key) по возрастанию разницы
                        # Пример 1: [8, 10, 16]
                        # Пример 2: [8, 10, 12, 14]
                        # Пример 3: Первая итерация - [], вторая - [8]
                        key
                        for key in sorted_keys
                        if abs(key - start_key) >= out_scope
                    )
                    if abs(next_key - result_key) >= out_scope
                )
        except StopIteration:
            # Если генератор не смог сформировать требуемое количество групп (groups)
            # Продолжаем поиск для следующей группы с минимальной разницей из списка sorted_keys
            # Для примера 3 на первой итерации возникнет исключение
            continue
        else:
            # Если генератор сформировал требуемое количество групп, то result_key содержит ключ
            # на последнюю группы из сформированных, которая содержит наибольшую разницу из
            # возможных минимальных
            # Пример 1: 16
            # Пример 2: 14
            # Пример 3: 8
            result = diff_ranges[result_key]
            # Досрочно выходим из цикла: for start_key in sorted_keys
            break
    return result
