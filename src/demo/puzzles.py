...
def get_max_from_min_difference(
    ...
    for start_key in sorted_keys:
        try:
            # Одношаговый генератор ищет группу с нужной величиной разницы.
            # Если найти не удалось, возникает исключение.
            # Пример 1: [8, 16]
            # Пример 2: [8, 14]
            # Пример 3: Первая итерация - [], вторая - [8]
            result_key: int = next(
                key for key in sorted_keys if abs(key - start_key) >= offset
            )
        except StopIteration:
            ...
            # Если генератор нашел группу, то result_key содержит ключ на группы,
            # которая содержит наибольшую разницу из возможных минимальных
            # Пример 1: 16
            # Пример 2: 14
            # Пример 3: 8
