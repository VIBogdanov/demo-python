...
def closest_amount(
    ...
    while query_buff:
        ...
        for next_sum, next_numbers in (
            ...
            # Можно просто:  if number > 0 and (current_sum + number) <= target
        ...
        def __call__(self, index: int, number: TNumber) -> None:
            ...
            if not (
                ...
                if (
                    ...
                    self.__ranges.clear()
                # Если накопленная сумма больше/меньше или равна,
                # то формируем список пар начальных и конечных индексов
...
def get_max_from_min_difference(
    ...
    Examples:
        1. groups=3, members=3, data=[1, 1, 1, 2, 2, 2, 2, 10, 10, 11] -> 1
        2. groups=3, members=3, data=[1, 1, 1, 2, 2, 2, 2, 10, 10] -> 8
        3. groups=2, members=3, data=[1, 1, 2, 2, 2, 3, 3] -> 1
        4. groups=2, members=3, data=[170, 205, 225, 190, 260, 130, 225, 160] -> 30
    ...
    diff_groups: list[tuple[int, int]] = sorted(
        (
            ...
            if a <= (len_data - size_groups) or a >= offset
        ),
...
def get_max_sequence_length(
    ...
    def sequence_length(
        ...
        # учесть начальные (A) и последние символы до и после искомого символа.
        ...
        for idx, symb in enumerate(sequence):
            ...
            if symb == symbol:
                # Если количество ранее найденных символов сравнялось с требуемым
                # При этом учитываем 'лишний' индекс -1
                if len(idx_store) - 1 == quantity:
        ...
        # в 'хвосте' после последнего найденного искомого символа. В данном примере это 'EEE'. При этом idx
